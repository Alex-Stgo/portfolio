...
class leg():
    class rate():
        class curve():
            def __init__(self,load=True):
                ...
                if load:
                    self.load()
            def load(self):
                self.data=excel.xls(r"static/files/curve.xls",usecols=list(self.columns.keys())).data
                self.data=self.data.rename(columns=self.columns)
                self.data=self.data.set_index('Plazo')
        types = {'f':'Fixed','v':'Variable'}
        def __init__(self,type,curve=None,level=None,fwd=False,fwd_period=28):
            if not curve is None:
                self.curve_obj=self.curve()
                self.curve=self.curve_obj.data[curve]
            self.level=level
            self.fwd=fwd
        ...
        def variable(cls,curve,fwd,fwd_period=28):
            return cls('v',curve=curve,fwd=fwd,fwd_period=fwd_period)
        def _get(self,n):
